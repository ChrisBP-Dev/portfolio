...
- [ ] Task 1: Configurar Firebase Emulator Suite (AC: #1, #2)
  - [ ] 1.1 Instalar `firebase-tools` como devDependency: `pnpm add -D firebase-tools`
...
- [ ] Task 4: Crear test data factories (AC: #5)
  ...
  - [ ] 4.2 Crear `src/test/factories/index.ts` que re-exporte todas las factories
  - [ ] 4.3 Crear `src/test/factories/project.ts` con `createProject(overrides?)` que retorna un objeto `Project` tipado
  - [ ] 4.4 Crear `src/test/factories/technology.ts` con `createTechnology(overrides?)` que retorna un objeto `Technology` tipado
...
- [ ] Task 5: Documentar en README (AC: #6)
  ...
  - [ ] 5.2 Documentar prerequisitos (Node 22+, pnpm 10+, Java 11+ para emuladores)
  - [ ] 5.3 Documentar cómo levantar emuladores y ejecutar tests
