...
- [ ] Task 1: TipTap JSON to HTML renderer (AC: #1, #3)
  - [ ] 1.1 Create `src/lib/utils/tiptap-renderer.ts` with `renderTipTapToHtml(json: string): string`
  - [ ] 1.2 Parse TipTap JSON, recursively walk nodes, emit HTML string. Handle all node types used by the editor: `doc`, `paragraph`, `heading` (levels 1-3), `bulletList`, `orderedList`, `listItem`, `codeBlock`, `blockquote`, `horizontalRule`, `hardBreak`, `image`
  - [ ] 1.3 Handle all mark types: `bold` → `<strong>`, `italic` → `<em>`, `strike` → `<s>`, `code` → `<code>`, `link` → `<a href="..." target="_blank" rel="noopener noreferrer">`
  - [ ] 1.4 Image nodes: `<img src="..." alt="..." loading="lazy" />` — use `attrs.src` and `attrs.alt`
  - [ ] 1.5 Code blocks: `<pre><code>...</code></pre>` — CSS class `blog-code-block` added for JetBrains Mono styling
...
- [ ] Task 3: Extend BaseLayout with OpenGraph props (AC: #4, #5)
  - [ ] 3.1 Add optional Props to `BaseLayout.astro`: `ogImage?: string`, `ogType?: string`, `ogDescription?: string`
  - [ ] 3.2 In `<head>`, add conditional OG meta tags: `og:title` (from existing `title` prop), `og:description` (from `ogDescription` ?? `description`), `og:image`, `og:type`, `og:url` (from canonical URL), `og:locale`, `og:site_name`
  - [ ] 3.3 Add Twitter Card meta tags: `twitter:card` (use `summary_large_image` when `ogImage` is present, else `summary`), `twitter:title`, `twitter:description`, `twitter:image`
  - [ ] 3.4 Existing pages that don't pass OG props remain unchanged (no regression)

- [ ] Task 4: Blog article i18n translations (AC: #1, #7)
  - [ ] 4.1 In `src/lib/i18n/translations.ts`, add keys:
    - `'blog.article.backToBlog'`: `{ es: '← Volver al Blog', en: '← Back to Blog' }`
    - `'blog.article.publishedOn'`: `{ es: 'Publicado el', en: 'Published on' }`

- [ ] Task 5: BlogContent.astro component (AC: #1, #2, #3)
  - [ ] 5.1 Create `src/components/blog/BlogContent.astro` — receives `contentHtml: string`
  - [ ] 5.2 Render sanitized HTML via `<div class="blog-content" set:html={contentHtml} />`
  - [ ] 5.3 Add scoped styles in `<style>` for `.blog-content` (using Astro's scoped styles with `:global()` for child selectors):
...
- [ ] Task 6: Blog article page — EN (AC: #1, #2, #4, #5, #8)
  - [ ] 6.1 Create `src/pages/blog/[slug].astro` — follow pattern of `src/pages/projects/[slug].astro`
...
- [ ] Task 8: Unit tests — TipTap renderer (AC: #1, #3)
  - [ ] 8.1 Create `src/lib/utils/__tests__/tiptap-renderer.test.ts`
  - [ ] 8.2 Test: paragraph with text → `<p>text</p>`
  - [ ] 8.3 Test: heading level 2 → `<h2>text</h2>`
  - [ ] 8.4 Test: bold + italic marks → `<strong><em>text</em></strong>`
  - [ ] 8.5 Test: link mark → `<a href="..." target="_blank" rel="noopener noreferrer">text</a>`
  - [ ] 8.6 Test: code block → `<pre class="blog-code-block"><code>code</code></pre>`
  - [ ] 8.7 Test: bullet list → `<ul><li><p>item</p></li></ul>`
  - [ ] 8.8 Test: ordered list → `<ol><li><p>item</p></li></ol>`
  - [ ] 8.9 Test: image node → `<img src="..." alt="..." loading="lazy" />`
  - [ ] 8.10 Test: blockquote → `<blockquote><p>text</p></blockquote>`
  - [ ] 8.11 Test: horizontal rule → `<hr />`
  - [ ] 8.12 Test: hard break → `<br />`
  - [ ] 8.13 Test: inline code mark → `<code>text</code>`
  - [ ] 8.14 Test: strikethrough mark → `<s>text</s>`
  - [ ] 8.15 Test: complex document with mixed nodes renders correctly
  - [ ] 8.16 Test: empty/invalid JSON → empty string
  - [ ] 8.17 Test: nested list items render correctly

- [ ] Task 9: Unit tests — HTML sanitization (AC: #1)
  - [ ] 9.1 Create `src/lib/utils/__tests__/sanitize-blog-html.test.ts`
  - [ ] 9.2 Test: allowed tags pass through (p, h2, a, img, pre, code, ul, ol, li, strong, em, blockquote, hr, br, s)
  - [ ] 9.3 Test: `<script>` tags stripped
  - [ ] 9.4 Test: `<iframe>` tags stripped
  - [ ] 9.5 Test: `onerror`, `onclick` event handlers stripped from attributes
  - [ ] 9.6 Test: `a[href]` with `javascript:` protocol stripped
  - [ ] 9.7 Test: `img[src,alt,loading]` allowed, `img[onerror]` stripped
  - [ ] 9.8 Test: full pipeline `sanitizeBlogHtml(renderTipTapToHtml(json))` produces safe output

- [ ] Task 10: E2E tests — blog article page (AC: #1, #2, #3, #4, #5, #7)
  - [ ] 10.1 Create `tests/e2e/blog-article.spec.ts` — follow pattern of `tests/e2e/project-detail.spec.ts`
  - [ ] 10.2 Test EN: navigate from `/blog` listing, click first card, verify article page loads at `/blog/[slug]`
  - [ ] 10.3 Test EN: page has `<h1>` with article title, `<time>` with date, reading time text
  - [ ] 10.4 Test EN: if cover image exists, `<img>` visible with `fetchpriority="high"`
  - [ ] 10.5 Test EN: content area `.blog-content` is visible with rendered HTML
  - [ ] 10.6 Test EN: back link navigates to `/blog`
  - [ ] 10.7 Test EN: page title includes article title and "ChrisBP"
  - [ ] 10.8 Test EN: OG meta tags present — `og:title`, `og:description`, `og:type` with value "article"
  - [ ] 10.9 Test EN: Twitter Card meta `twitter:card` present
  - [ ] 10.10 Test ES: page loads at `/es/blog/[slug]` with Spanish content
  - [ ] 10.11 Test ES: back link navigates to `/es/blog`
