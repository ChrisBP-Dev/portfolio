...
- [ ] Task 1: Fix cover image lifecycle in BlogForm edit save (AC: #5)
  ...
  - [ ] 1.3 Handle `processed.image === null` case (cover removed): `updateDoc` with `coverImage: deleteField()` to clear from Firestore — import `deleteField` from `firebase/firestore`
...
**Import required:** `imageService` must be added to BlogForm.svelte imports. Currently only `UploadHandle` type is imported from `image-service.ts` (line 5). Add: `import { imageService, type UploadHandle } from '../../lib/firebase/image-service';`
